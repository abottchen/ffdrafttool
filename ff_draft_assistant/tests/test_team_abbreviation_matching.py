...
class TestTeamAbbreviationMatching:
    ...
    async def test_team_abbreviation_variations(self):
        ...
        with patch('src.tools.mcp_tools.get_player_rankings', new_callable=AsyncMock) as mock_rankings:
            ...
            assert "Christian McCaffrey" not in available_names, "McCaffrey (SF/SFO) should be filtered out"
            assert "Josh Jacobs" not in available_names, "Jacobs (GB/GBP) should be filtered out"
            assert "Alvin Kamara" not in available_names, "Kamara (NO/NOS) should be filtered out"
            assert "Rhamondre Stevenson" not in available_names, "Stevenson (NE/NEP) should be filtered out"
            assert "Mike Evans" not in available_names, "Evans (TB/TBB) should be filtered out"

            # This player should NOT be filtered out
            assert "Saquon Barkley" in available_names, "Barkley should be available (not drafted)"
    ...
    async def test_exact_team_match_still_works(self):
        ...
        with patch('src.tools.mcp_tools.get_player_rankings', new_callable=AsyncMock) as mock_rankings:
            ...
            assert "Patrick Mahomes" not in available_names, "Mahomes should be filtered out (exact match)"
    ...
    async def test_name_only_matching_still_works(self):
        ...
        with patch('src.tools.mcp_tools.get_player_rankings', new_callable=AsyncMock) as mock_rankings:
            ...
            assert "Tom Brady" not in available_names, "Brady should be filtered out (name-only match)"
    ...
    def test_team_matching_edge_cases(self):
        ...
        test_cases = [
            ...
            ("LA", "LAC", True),    # first 2 characters match (this might be too broad, but let's see)
        ]

        for drafted_team, rankings_team, should_match in test_cases:
            # Simulate the matching logic
            teams_match = (
                drafted_team in rankings_team or
                rankings_team in drafted_team or
                (len(drafted_team) >= 2 and len(rankings_team) >= 2 and
                 drafted_team[:2] == rankings_team[:2])
            )

            assert teams_match == should_match, \
