...
class TestDraftStrategyAdvice:
    ...
    def sample_available_players(self):
        ...
        players = []
        positions = [Position.QB, Position.RB, Position.WR, Position.TE, Position.K, Position.DST]
    ...
    def test_early_round_strategy_advice(self, team_analysis, early_draft_team, sample_available_players):
    ...
    def test_flex_depth_analysis(self, team_analysis, sample_available_players):
        ...
        assert ("flex" in opportunities_text.lower() or "depth" in opportunities_text.lower() or
                "flex" in primary_needs_text.lower())

        # FLEX analysis should show limited options
        assert advice["flex_analysis"]["total_flex_options"] <= 4

    def test_late_round_strategy(self, team_analysis, late_draft_team, sample_available_players):
    ...
    def test_balanced_team_advice(self, team_analysis, mid_draft_team, sample_available_players):
        ...
        skill_position_needs = [n for n in advice["primary_needs"]
    ...
    def test_strategy_advice_structure(self, team_analysis, early_draft_team, sample_available_players):
        ...
        expected_keys = [
            "primary_needs", "warnings", "opportunities",
            "position_limits", "flex_analysis", "strategy_notes"
        ]
        ...
        flex_keys = ["total_flex_options", "rb_depth", "wr_depth", "te_depth", "best_flex_option"]
        for key in flex_keys:
            assert key in advice["flex_analysis"]

    def test_recommendations_based_on_round(self, team_analysis, early_draft_team, sample_available_players):
        ...
        late_advice = team_analysis.get_draft_strategy_advice(
            early_draft_team, sample_available_players, current_round=13, total_rounds=15
        )
        ...
        assert "kicker" in late_strategy or "defense" in late_strategy or "late" in late_strategy
