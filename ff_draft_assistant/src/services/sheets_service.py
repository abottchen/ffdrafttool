...
class SheetsProvider(ABC):
    ...
    @abstractmethod
    async def write_range(self, sheet_id: str, range_name: str, values: List[List[Any]]) -> bool:
...
class GoogleSheetsProvider(SheetsProvider):
    ...
    def __init__(self, credentials_file: Optional[str] = None, token_file: Optional[str] = None):
        if not GOOGLE_AVAILABLE:
            raise ImportError("Google API dependencies not available. Install with: pip install google-api-python-client google-auth-httplib2 google-auth-oauthlib")
        ...
        self.credentials_file = credentials_file or os.getenv('GOOGLE_CREDENTIALS_FILE', str(default_creds))
        self.token_file = token_file or os.getenv('GOOGLE_TOKEN_FILE', str(default_token))
        self.service = None
    ...
    async def _get_service(self):
        ...
        self.service = await asyncio.get_event_loop().run_in_executor(self.executor, _auth)
        return self.service
    ...
    async def read_range(self, sheet_id: str, range_name: str) -> List[List[Any]]:
        ...
        try:
            ...
            values = await asyncio.get_event_loop().run_in_executor(self.executor, _read)
            logger.info(f"Successfully read {len(values)} rows from sheet {sheet_id}, range {range_name}")
            return values
    ...
    async def write_range(self, sheet_id: str, range_name: str, values: List[List[Any]]) -> bool:
        ...
        try:
            ...
            # Execute API call in thread pool
            success = await asyncio.get_event_loop().run_in_executor(self.executor, _write)
            return success
...
class SheetsService:
    ...
    def __init__(self, provider: SheetsProvider, draft_rules: DraftRules = None, use_cache: bool = True):
    ...
    def _find_team_by_pick_position(self, teams: List[Dict], pick_number: int, total_teams: int, round_num: int = None) -> Optional[Dict]:
        """
    ...
    async def read_draft_data(self, sheet_id: str, range_name: str, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.cache:
                use_incremental, first_round_to_read = self.cache.should_use_incremental_read(
                ...
                if use_incremental:
                    ...
                    if cached_state:
                        logger.info(f"Attempting incremental read from round {first_round_to_read}")
                        incremental_range = self.cache.get_incremental_range(sheet_id, range_name, first_round_to_read)

                        try:
                            incremental_data = await self.provider.read_range(sheet_id, incremental_range)
                            if incremental_data and len(incremental_data) > 0:
                                # Parse the incremental data and merge with cache
                                new_picks = self._parse_incremental_picks(incremental_data, first_round_to_read)
                                merged_state = self.cache.merge_incremental_data(sheet_id, cached_state, new_picks)

                                # Update cache with merged state
                                self.cache.update_cache(sheet_id, range_name, merged_state)
                                logger.info(f"Successfully merged {len(new_picks)} new picks with cached data")
                                return merged_state
                            else:
                                logger.info("No new data found, returning cached state")
                                return cached_state
                        except Exception as e:
                            logger.warning(f"Incremental fetch failed, falling back to full fetch: {e}")
            ...
            while col_idx < max(len(owner_row), len(team_row)):
                ...
                if not owner or not team or owner.lower() in ['player', 'pos', 'position', ''] or team.lower() in ['player', 'pos', 'position', '']:
                    col_idx += 1
                    continue
                ...
                is_position_col = (
                    not next_team and  # Team name is empty
                    (not next_owner or  # Owner is empty OR
                     next_owner in ['*', '**', '***', '****'] or  # Owner is asterisks OR
                     next_owner.lower() in ['pos', 'position'] or  # Owner is position header OR
                     len(next_owner) <= 5 and not next_owner.replace('*', '').replace(' ', '').isalpha())  # Short notes/symbols
                )
                ...
                if is_position_col:
                    teams.append({
                        ...
                        'owner_identifier': owner.lower().replace(' ', ''),  # For matching
                        'team_identifier': team.lower().replace(' ', '').replace("'", ''),  # For matching
            ...
            for pick_idx, sheet_pick in enumerate(all_sheet_picks):
                ...
                correct_team = self._find_team_by_pick_position(teams, pick_number, len(teams), round_num)
                ...
                picks.append({
                    ...
                    'column_team': sheet_pick['team_found']['team_name'],  # Track which column it came from for debugging
            ...
            current_round = current_round if 'current_round' in locals() else (completed_rounds + 1)
            ...
            for round_num in range(1, completed_rounds + 2):  # Include current round
                ...
                if round_picks or round_num == current_round:
                    round_analysis[round_num] = {
                        'round_type': self.draft_analyzer.rules.get_round_type(round_num).value,
                        ...
                        'strategy': self.draft_analyzer.get_draft_strategy_for_round(round_num)
                    }
            ...
            result = {
                ...
                "current_round_strategy": self.draft_analyzer.get_draft_strategy_for_round(current_round),
    ...
    def _parse_incremental_picks(self, incremental_data: List[List[Any]], start_round: int) -> List[Dict[str, Any]]:
        """
        ...
        for row_idx, row in enumerate(incremental_data):
            ...
            while col_idx < len(row) - 1:
                player = row[col_idx].strip() if col_idx < len(row) and row[col_idx] else ""
                position = row[col_idx + 1].strip() if col_idx + 1 < len(row) and row[col_idx + 1] else ""
        ...
        logger.info(f"Parsed {len(new_picks)} picks from incremental data starting at round {start_round}")
        return new_picks
