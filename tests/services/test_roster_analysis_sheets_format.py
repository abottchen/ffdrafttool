...
class TestRosterAnalysisSheetsFormat:
    ...
    async def test_roster_analysis_with_column_team_field(self):
        ...
        with patch('src.tools.mcp_tools.get_player_rankings') as mock_rankings:
            ...
            result = await suggest_draft_pick(mock_draft_state, team_name="Test Team")
            ...
            assert not any("Christian McCaffrey" in player for player in all_players), \
    ...
    async def test_roster_analysis_mixed_field_formats(self):
        ...
        mock_draft_state = {
            ...
            "teams": [
                {"team_name": "Test Team", "owner": "Test Owner"}
            ],
            "current_pick": 3
        }
